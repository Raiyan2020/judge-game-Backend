...
class LegalCaseOpinionServices
{
    ...
    public function createOpinion($request)
    {
        try {
            ...
            // Part 3: if the judge was AWAITING this exact role's opinion
            // (`awaiting_opinion` = `defendant_lawyer` | `consultant`), this
            // submission answers it — clear the gate so the judge can act again.
            // The `case_opinion` judge notification below still fires. Captured
            // before the clear so the flag reflects the pre-submission state.
            $clearsAwaiting = ! empty($legalCase->awaiting_opinion)
                && $role === $legalCase->awaiting_opinion;
            ...
            // Part 3: the awaited party has now responded — clear the gate inside
            // the transaction so it commits atomically with their opinion, freeing
            // the judge to take the next action.
            if ($clearsAwaiting) {
            ...
            DB::afterCommit(function () use ($legalCase, $legalCaseOpinion, $isOfficialFiling, $clearsAwaiting) {
                ...
                if ($isOfficialFiling) {
                    ...
                    // The status flipped pending_lawyer → new — push the committed
                    // state so an open screen re-renders. Fail-soft in the helper.
                    LegalCaseService::broadcastCaseUpdated($legalCase);
                    return;
                }

                // Part 3: the awaited party just answered and the `awaiting_opinion`
                // gate cleared — push the committed state so the judge's open case
                // screen re-renders with their actions unblocked, live. Fired FIRST,
                // before the notification sends below: those are not try/catch'd, so
                // a throwing FCM send must not swallow this gate-clear nudge (the
                // primary realtime deliverable). Fail-soft in the helper.
                if ($clearsAwaiting) {
                    LegalCaseService::broadcastCaseUpdated($legalCase);
                }
